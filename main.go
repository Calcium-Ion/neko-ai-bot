...
func CheckStatus(tgBog tgbotapi.BotAPI) {
	...
	//		respBody, err := util.DoRequest("https://nekoapi.com", fmt.Sprintf("/api/log/stat?&type=0&token_name=&model_name=&start_timestamp=%d&end_timestamp=%d", currentUnixTimestamp-60, currentUnixTimestamp), conf.Conf.AccessToken)
	//		if err != nil {
	//			log.Println(err)
	//			continue
	//		}
...
func main() {
	...
	for update := range updates {
		...
		if update.Message != nil {
			...
			} else if strings.HasPrefix(update.CallbackQuery.Data, "/status") {
				baseUrl := strings.Split(update.CallbackQuery.Data, " ")[1]
				bot.Status(*update.CallbackQuery.Message, *tgBot, baseUrl)
			}
		}
	}
}
